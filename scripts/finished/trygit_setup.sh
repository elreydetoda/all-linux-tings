...
function run_nginx_docker(){

  # set a custom ngingx config
...
function download_try_web_assets(){
  ...
  ##################################################
  # got all these assets by pulling this file: https://web.archive.org/web/20170110114657/https://d13jv82ekraqyq.cloudfront.net/assets/application-8c99a6177b53973aec4c34bb07a79157.css
  #   just as plaintext ( copy paste ), and then running the following for loop to print it out in the format to pull it down, and pasted it in this script
  #   for i in $(ag --nocolor --nobreak --nofilename -o "//web\.archive\.org[\.\w/\:-]+(?=[\"|\)|'])") ; do timestamp="$(grep -oP '\d{14}' <<< "${i}" | grep -oP '^\d{6}')" ; urlz="$(grep -oP 'https://.*')" <<< "${i}" ; printf 'docker run --rm -it -v "$PWD/websites":/websites hartator/wayback-machine-downloader %s --to %s --from %s\n' "${urlz}" "${timestamp}" "${timestamp}" ; done
...
function download_initial_try_web(){
...
function main(){
  # inspired by the way try.github.io used to look: https://web.archive.org/web/20160503070318/https://try.github.io/levels/1/challenges/1
  base_dir='websites'
  try_git_dir="${base_dir}/try.github.io"
  download_initial_try_web
  download_try_web_assets
